...
public class GetProjectUseCaseTests {
	...
	@Test
	@DisplayName("it should be able to get one project")
	void getProjectsSuccessCase() {
		...
		).thenReturn(Optional.of(baseContentProject));

		assertDoesNotThrow(
			() ->
				this.getProjectUseCase.exec(
						baseContentProject.getId().toString(),
						user
					)
		);
	}
